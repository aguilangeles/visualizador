...
abstract class EMongoDocument extends EMongoEmbeddedDocument
{
	...
	/**
	 * distinct all documents satisfying the specified condition.
	 * See {@link find()} for detailed explanation about $condition and $params.
	 * @param array|EMongoCriteria $condition query criteria.
	 * @return integer Count of all documents satisfying the specified condition.
	 * @since v1.0
	 */
	public function distinct($key,$criteria=null)
	{
		Yii::trace(get_class($this).'.distinct()','ext.MongoDb.EMongoDocument');
		$this->applyScopes($criteria);
		$collName = $this->getCollectionName();
		$conds = $criteria->getConditions();
		return $this->db->command(array("distinct"=>$collName,"key"=>"$key","query"=>$conds));
	}
	 ...
	 * @since v1.0
	 */
	public function group($keys=null, $initial=array('items'=>array()), $reduce='function (obj, prev) {}', $criteria=null, $fields=null)
	{
		Yii::trace(get_class($this).'.group()','ext.MongoDb.EMongoDocument');
		$this->applyScopes($criteria);
		$collName = $this->getCollectionName();
		$conds = $criteria->getConditions();
		//$initial = array("items" => array());
		//$reduce = "function (obj, prev) { prev.items.push(obj.CMETA_CAJA);prev.items.push(obj.CMETA_SUCURSAL); }";
		return $this->getCollection()->group($keys,$initial,$reduce,$conds);
	}
