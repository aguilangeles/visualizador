...
//			$("#filterLevel4").hide();
//		}                
//		$.ajax({url: "<?php echo Yii::app()->request->hostinfo?>/searchMetaCarat/searchMetaCarat",
...
function toggleRotulosSearch()
{
	...
	if (rotuloId != "0")
	{
		...
		$.ajax({url: "<?php echo Yii::app()->request->hostinfo?>/site/searchRotulos",
		context: document.body,
		type: "POST",
...
function SearchDocs(page)
{
	...
	var dataCMeta = $("input[id='CMETA_']").map(function(){return $(this).val();}).get();
	var CmetaFields ='';
	for (i in dataCMeta)
	{
		CmetaFields = CmetaFields +"&CMETA_["+i+"]="+dataCMeta[i];
	}
	var dataOcrMeta = $("input[id='OCR_']").map(function(){return $(this).val();}).get();
	var OcrFields ='';
...
function SearchGralDocs(page,docType)
{
	...
	$("#filterRestict :checked").each(function() {Docs.push($(this).val());});
	if (Docs == ""){
...
function openMetaForm(id){     		   	
		$("#meta_form").dialog( "option", "buttons", [ 
			{ text: "Actualizar", click: function() { editImage(id); } },
