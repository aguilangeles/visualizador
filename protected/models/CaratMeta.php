...
class CaratMeta extends CActiveRecord
{
	...
	public $documento;//http://www.codeinphp.com/frameworks/yii/using-filters-yii-cgridview-relations/
	//permite filtrar el tipo de documento por nombre y no por id
	...
	public function rules()
	{
		...
		return array(
		  ...
		  // probar hacer otra rule separada para tipodoc
	...
	public function validateRotuloAsoc($attribute, $params)
	{
		...
		//desabilito esta opcion porque no se sabe para qué esta -> maguilar
...
//		}
	 ...
	 * @author GDM
	 */
	public static function getCMetaLabelByName($carat_meta_desc, $docId,
					    $replace_cmeta = false)
	{
		$c = new CDbCriteria();
		if ($replace_cmeta) {
	...
	public static function getInputHtml($conditions)
	{
		...
		foreach ($conditions as $condition) {
			if ($condition->field != 'docType' && substr($condition->field, 0, 4) != 'OCR_' && $condition->field != 'c1' && $condition->field != 'c2' && $condition->field != 'c3' && $condition->field != 'c4') {
	...
	protected static function updateMetaCaratXML($paths, $new_data,
					      $result = array())
	{
		$search = array('/', '\\', '|');
		$xmls = array();
