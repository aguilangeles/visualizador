...
//			else
//			{
//				$doc = DocTypes::model()->findByPk($docLevel1);
//				$docsLevel1 = array($doc->doc_type_id=>$doc->doc_type_desc);
//			}
...
//                      }
			...
			for($x=0; $x < count($items['images']);$x++)
			{
				...
				if($tipo == 'OZ')
					{
						$content = $content.'<td>'.CHtml::link(CHtml::image('/images/image.png'),'#', 						
                                array('style'=>'margin-right:5px;','onClick'=>'showImage("'.$items['id'].'","'.$x.'"); return false;',));	
					}
				else
					{
						$content = $content.'<td>'.CHtml::link(CHtml::image('/images/image.png'),'#', 						
                                array('style'=>'margin-right:5px;','onClick'=>'showImageSmall("'.$items['id'].'","'.$x.'"); return false;',));	
                                                
                                                
					}
							
                $edit_image = (Yii::app()->user->isAdmin) ? CHtml::link(CHtml::image('/images/edit_icon.png'),'#', 						
                                array('onClick'=>'openMetaForm("'.$items['images'][$x]->id.'"); return false;',)):'';
...
//		$criteria = new EMongoCriteria();
//		$criteria->addCond('fileName', '==', $fileName);
//		$idc = Idc::model()->find($criteria);
//		$criteria2 = new EMongoCriteria();
//		$criteria2->addCond('c1', '==', $idc->c1);
//		$criteria2->setSort(array('order'=>EMongoCriteria::SORT_ASC));
//		$keys =array('c1');
//		$initial = array("images" => array(),"index"=>0,"info"=>array());
//		$reduce = "function (obj, prev) { prev.images.push(obj.fileName);prev.info.push(obj.order);}";
//		$group = Idc::model()->group($keys,$initial,$reduce,$criteria2);
//		$group = $this->orderResults($group);
//		$count = $group['count'];
//		$result = $group['retval'][0]['images'];
//		if ($newPos > $count)
//		{
//			echo "Error, el valor no puede ser mas grande que ".$count;
//		}
//		else
//		{
//			if ($oldPos > $newPos)
...
//					{
//						$c = new EMongoCriteria();
//						$c->addCond('fileName', '==', $result[$i]);
//						$modifier = new EMongoModifier();
//						$modifier->addModifier('order', 'set', $newPos);
//						Idc::model()->updateAll($modifier, $c);
//					}
//					else
//					{
//						$c = new EMongoCriteria();
//						$c->addCond('fileName', '==', $result[$i]);
//						$modifier = new EMongoModifier();
//						$modifier->addModifier('order', 'set', $i+2);
//						Idc::model()->updateAll($modifier, $c);
//					}
//					
//				}
...
//						$c->addCond('fileName', '==', $result[$i-1]);
//						$modifier = new EMongoModifier();
//						$modifier->addModifier('order', 'set', $i-1);
//						Idc::model()->updateAll($modifier, $c);
//					}
//				}
//			}
